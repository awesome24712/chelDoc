...
template<class T> bool CTree<T>::shouldDestroy() const {
	bool bShouldDestroy = (bool) m_pNode && !numExternalReferences();
	
	for (int i = 0; bShouldDestroy && i < numChildren(); i++)
		bShouldDestroy = child(i).shouldDestroy();
	
	return bShouldDestroy;
}

template<class T> void CTree<T>::destroyTree() {
...
template<class T> void CTree<T>::addChild(CTree<T>& otherTree) {
	AssertTrue(otherTree.isRoot(), "For CTree<T>::addChild(CTree<T>& otherTree), otherTree has no parent");
...
template<class T> int CTree<T>::height() const {
	int result = 1;
	
	int maxSubTreeHeight = 0;
	for (int i = 0; i < numChildren(); i++) {
		int nextHeight = child(i).height();
		if (nextHeight > maxSubTreeHeight)
			maxSubTreeHeight = nextHeight;
	}
	result += maxSubTreeHeight;
	return result;
}
